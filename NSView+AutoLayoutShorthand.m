// NSView+AutoLayoutShorthand.m semver:0.2
...
+ (instancetype)viewAttrWithView:(NSView *)view attr:(NSLayoutAttribute)attr;
@end
...
@implementation NSView (AutoLayoutShorthand)
...
- (void)als_addConstraints:(NSDictionary *)constraints
{
    for (NSString *constraintKey in constraints) {
        NSView           *firstItem       = self;
        NSLayoutAttribute firstAttribute  = [NSView _jr_parseLayoutAttributeName:constraintKey];
        NSLayoutRelation  relation        = [NSView _jr_parseLayoutRelation:constraintKey];
        NSView           *secondItem      = nil;
        NSLayoutAttribute secondAttribute = NSLayoutAttributeNotAnAttribute;
        CGFloat           multiplier      = 1.0;
        CGFloat           constant        = 0.0;
        NSLayoutPriority  priority        = NSLayoutPriorityRequired;
        ...
        if (secondItem) {
            NSView *closestCommonSuperview = nil;

            NSView *rhsViewItr = self.superview;

            while (!closestCommonSuperview && rhsViewItr) {
                NSView *lhsViewItr = secondItem;
